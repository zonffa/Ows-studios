...
// Conexión
$mysqli = new mysqli("localhost", "root", "", "ows2");
...
if (isset($_POST['asignar_unidad'])) {
    ...
    if ($res->num_rows > 0) {
        $mysqli->query("UPDATE usuarios_unidades SET id_unidad=$unidad, fecha_asignacion=NOW() WHERE id_usuario=$usuario");
    } else {
        $mysqli->query("INSERT INTO usuarios_unidades (id_usuario,id_unidad,fecha_asignacion) VALUES ($usuario,$unidad,NOW())");
    }
}
...
$pagos = $mysqli->query("SELECT p.*, u.nombre FROM pagos p JOIN usuarios u ON p.id_usuario=u.id_usuario ORDER BY p.fecha_pago DESC");
$asistencias = $mysqli->query("SELECT a.*,u.nombre FROM asistencias a JOIN usuarios u ON a.id_usuario=u.id_usuario ORDER BY fecha DESC");
...
<header><h1>Admin | VoleuCoorp</h1></header>
...
<?php
$uid=$u['id_usuario'];
$q=$mysqli->query("SELECT direccion FROM usuarios_unidades uu JOIN unidades_habitacionales u ON u.id_unidad=uu.id_unidad WHERE id_usuario=$uid");
echo ($q->num_rows? $q->fetch_assoc()['direccion'] : 'Sin asignar');
?>
</td>
<td>
<form method="POST" class="form-asignar">
  ...
  <select name="id_unidad" required>
    ...
    <?php foreach($lista_unidades as $lu): ?>
      <option value="<?=$lu['id_unidad']?>"><?=$lu['direccion']?></option>
    <?php endforeach; ?>
  </select>
  <button class="btn-aceptar" name="asignar_unidad">Asignar</button>
</form>
</td>
</tr>
<?php endwhile; ?>
...
<td><?=nl2br($e['motivo'])?></td>

<td>
<?php if($e['comprobante']): ?>
<a class="btn" target="_blank" href="../backoffice/exoneraciones/<?=$e['comprobante']?>">Ver archivo</a>
<?php else: ?>Sin archivo<?php endif;?>
</td>
...
<thead>
<tr><th>ID</th><th>Usuario</th><th>Correo</th><th>Archivo</th><th>MIME</th><th>Tamaño</th><th>Fecha</th><th>Ver</th></tr>
</thead>
<tbody>
...
<td><?=$d['nombre']?></td>
<td><?=$d['correo']?></td>
<td><span class="badge"><?=$d['nombre_original']?></span></td>
<td><?=$d['mime']?></td>
<td><?=round($d['tamano']/1024,1)?> KB</td>
<td><?=$d['fecha_subida']?></td>
<td><a class="btn" href="../<?=$d['ruta']?>" target="_blank">Abrir</a></td>
</tr>
<?php endwhile; ?>
